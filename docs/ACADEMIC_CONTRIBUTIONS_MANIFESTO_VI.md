...
*Hệ quả khoa học:* Công thức này đảm bảo **Biên độ quyết định (Margin)** được mở rộng tối đa, tăng cường tính bền bỉ trước các truy vấn gây nhiễu.

---

### Đóng góp 2: Điều khiển Bằng chứng dựa trên Lý thuyết Thông tin (Information-Theoretic Evidence Control)
**Vấn đề SOTA:** GraphRAG hiện hành duyệt đồ thị theo cấu trúc lân cận, dẫn đến cửa sổ ngữ cảnh LLM bị nhồi nhét tài liệu rác (Noise insertion).
**Đột phá:** Nghiên cứu này định nghĩa lại quá trình duyệt đồ thị (Graph Traversal) thành một bài toán **Giảm độ bất định theo chuỗi (Sequential Uncertainty Reduction)**.

**Định nghĩa Toán học:**
Tại mỗi bước duyệt đồ thị, một nút $v$ chỉ được đưa vào tập bằng chứng $\mathcal{P}_t$ nếu nó tối đa hóa hàm Hữu dụng Biên (Marginal Utility):
$$ \Delta \mathcal{U}(v) = \text{Sim}(v, \mathcal{Z}(q)) + \beta \cdot \text{IG}(v \mid \mathcal{P}_t) - \gamma \cdot \text{Redundancy}(v, \mathcal{P}_t) $$
Trong đó, **Lợi ích Thông tin Biên (Marginal Information Gain - IG)** đo lường sự giảm Entropy (độ mù mờ) $H(\text{Path} \mid \mathcal{P}_t)$.

---

### Đóng góp 3: Khung Cách ly Ảo giác (Hallucination Isolation Framework)
**Vấn đề SOTA:** Thiếu chuẩn mực đo lường mức độ "Bám sát bằng chứng" (Groundedness).
**Đột phá:** Nghiên cứu đưa ra cơ chế phân tích lỗi **Phân rã Lỗi chuyên sâu (Deep Error Decomposition)**, chia tách thành các nhóm lỗi gốc ($E_{\text{Ont}}$, $E_{\text{Trav}}$, $E_{\text{Gnd}}$). 

**Lý thuyết Xác suất:**
Cô lập thành công do may mắn (ảo giác trả lời đúng) thông qua việc phân rã xác suất đúng $P(Y=1)$:
$$ P(Y=1) = P(Y=1 \mid S_{\text{grounded}})P(S_{\text{grounded}}) + P(Y=1 \mid \neg S_{\text{grounded}})P(\neg S_{\text{grounded}}) $$
*Hệ quả khoa học:* Khẳng định **Độ chính xác trên Căn cứ ($Prec_{\text{gnd}}$)** là thước đo thực sự về tính xác định và độ tin cậy của hệ thống.
