...
1.  **Phổ SOTA đối chứng:**
    *   **Nhóm Cấu trúc tri thức:** **Microsoft GraphRAG** (Duyệt đồ thị toàn cục) và **G-Retriever** (Graph + LLM).
    *   **Nhóm Cơ chế truy xuất:** **FLARE** (Active Retrieval) và **HyDE** (Hypothetical Embeddings).
    *   **Nhóm Quy trình lập luận:** **Self-RAG** (Self-reflection), **Tree of Thoughts (ToT)** (Reasoning as Search), và **Corrective RAG (CRAG)**.
    *   **Nhóm Tối ưu hệ thống:** **DSPy** (Declarative Self-improving Programs).
2.  **Tập dữ liệu chuẩn (Benchmark Datasets):** Thực hiện thực nghiệm trên **HotpotQA**, **2WikiMultiHopQA**, **MuSiQue** và **StrategyQA**.
3.  **Chỉ số đo lường vượt trội:**
    *   **Reasoning Efficiency:** Số bước nhảy tri thức (Hops) và lượng Token tiêu thụ trung bình.
    *   **Hallucination Rate:** Tần suất xuất hiện thông tin không có trong đồ thị/văn bản gốc.
    *   **Pareto Frontier:** So sánh đa mục tiêu giữa Accuracy, Latency và Cost.
